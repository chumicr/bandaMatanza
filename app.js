import { NAV_ITEMS as SITE_NAV_ITEMS, MEDIA as SITE_MEDIA, SITE } from './src/data/site.js';
import { CURRENT_DIRECTOR as SITE_CURRENT_DIRECTOR, CURRENT_BOARD as SITE_CURRENT_BOARD, VERIFIED_PRESIDENTS as SITE_VERIFIED_PRESIDENTS } from './src/data/people.js';
import { EDUCATION_PILLARS, EDUCATION_STAGES } from './src/data/education.js';
import { resolveRoute } from './src/utils/router.js';
import { renderNavigation, setupNavigation, setActiveNavigation } from './src/components/navigation.js';
import { setupRevealAnimations } from './src/components/animations.js';
import { page, title } from './src/pages/helpers.js';

const NAV_ITEMS = SITE_NAV_ITEMS;
const MEDIA = SITE_MEDIA;
const img = MEDIA;
const GOOGLE_MATRICULA_FORM_URL = SITE.googleEnrollmentFormUrl;
const CONTACT_EMAIL = SITE.contactEmail;
const ASSOCIATION_FOUNDATION_YEAR = SITE.foundationYear;
const CURRENT_DIRECTOR = SITE_CURRENT_DIRECTOR;
...
function setupHomeAnnouncements() {
  ...
  const featureGrid = section?.querySelector('.feature-grid');
  if (!section || !featureGrid) return;
  const associationYears = getAssociationYears();
  const announcements = [
    ['La banda', 'Sonar juntos cambia todo.', 'Conoce nuestro recorrido, los directores y los proyectos que nos mueven.', '#banda', 'Conocer la banda', img.concert],
    ['La escuela', 'Tu primer compás.', 'Clases para todas las edades, desde iniciación hasta conjunto instrumental.', '#escuela', 'Explorar la escuela', img.music],
    ['Nuestra historia', 'Una historia que sigue sonando.', `Más de ${associationYears} años de música, aprendizaje y momentos compartidos en La Matanza.`, '#nuestra-historia', 'Conocer nuestra historia', img.people],
    ['Próximamente', 'La música también se encuentra.', 'Consulta el calendario y descubre nuestros próximos conciertos y actividades.', '#calendario', 'Ver calendario', img.classroom]
  ];
  featureGrid.className = 'announcement-carousel';
  featureGrid.innerHTML = `<div class="announcement-viewport"><div class="announcement-track">${announcements.map(([label, titleText, text, href, cta, image], index) => `<article class="announcement-slide${index === 0 ? ' is-active' : ''}" style="background-image:url('${image}')"><div><small>${label}</small><h3>${titleText}</h3><p>${text}</p><a class="text-link" href="${href}">${cta} ↗</a></div></article>`).join('')}</div></div><div class="announcement-controls"><button type="button" class="announcement-arrow announcement-prev" aria-label="Anuncio anterior"><span aria-hidden="true">‹</span></button><div class="announcement-dots">${announcements.map((_, index) => `<button type="button" class="announcement-dot${index === 0 ? ' is-active' : ''}" aria-label="Ver anuncio ${index + 1}" aria-pressed="${index === 0}"></button>`).join('')}</div><button type="button" class="announcement-arrow announcement-next" aria-label="Anuncio siguiente"><span aria-hidden="true">›</span></button></div>`;
  const track = featureGrid.querySelector('.announcement-track');
  const slides = [...featureGrid.querySelectorAll('.announcement-slide')];
  const dots = [...featureGrid.querySelectorAll('.announcement-dot')];
  let current = 0;
  let startX = null;
  const move = (direction) => {
    ...
    dots.forEach((dot, index) => { dot.classList.toggle('is-active', index === current); dot.setAttribute('aria-pressed', String(index === current)); });
  };
  featureGrid.querySelector('.announcement-prev')?.addEventListener('click', () => move(-1));
  featureGrid.querySelector('.announcement-next')?.addEventListener('click', () => move(1));
  dots.forEach((dot, index) => dot.addEventListener('click', () => { current = index - 1; move(1); }));
  const viewport = featureGrid.querySelector('.announcement-viewport');
  viewport?.addEventListener('pointerdown', event => { startX = event.clientX; viewport.setPointerCapture?.(event.pointerId); });
  viewport?.addEventListener('pointerup', event => { if (startX === null) return; const distance = event.clientX - startX; if (Math.abs(distance) > 45) move(distance < 0 ? 1 : -1); startX = null; });
  window.announcementTimer = setInterval(() => move(1), 8000);
}

function setupGalleryLightbox(root) {
  const lightbox = root.querySelector('[data-gallery-lightbox]');
  if (!lightbox) return;
  if (window.galleryKeydownHandler) document.removeEventListener('keydown', window.galleryKeydownHandler);
  document.querySelectorAll('body > [data-gallery-lightbox]').forEach((node) => node.remove());
  lightbox.remove();
  ...
  prevButton.className = 'gallery-lightbox-nav gallery-lightbox-prev';
  nextButton.className = 'gallery-lightbox-nav gallery-lightbox-next';
  ...
  stage.append(image);
  zoomOutButton.className = 'gallery-lightbox-tool';
  zoomInButton.className = 'gallery-lightbox-tool';
  fullscreenButton.className = 'gallery-lightbox-tool';
  [zoomOutButton, zoomInButton, fullscreenButton].forEach((button) => { button.type = 'button'; });
  zoomOutButton.setAttribute('aria-label', 'Reducir imagen');
  zoomInButton.setAttribute('aria-label', 'Ampliar imagen');
  fullscreenButton.setAttribute('aria-label', 'Pantalla completa');
  zoomOutButton.textContent = '−';
  zoomInButton.textContent = '+';
  fullscreenButton.textContent = '⛶';
  closeButton?.setAttribute('aria-label', 'Cerrar visor');
  toolbar.append(counter, zoomOutButton, zoomInButton, fullscreenButton);
  if (closeButton) toolbar.append(closeButton);
  lightbox.append(toolbar, prevButton, nextButton);
  const applyZoom = () => {
    const isZoomed = zoom > 1;
    stage.classList.toggle('is-zoomed', isZoomed);
    if (isZoomed) {
      image.style.setProperty('width', `${zoom * 100}%`, 'important');
  ...
  const setZoom = (amount) => { zoom = Math.min(3, Math.max(1, amount)); applyZoom(); };
  const lockPage = () => {
    document.documentElement.classList.add('gallery-lightbox-open');
    document.body.classList.add('modal-open', 'gallery-lightbox-open');
  };
  const unlockPage = () => {
    document.documentElement.classList.remove('gallery-lightbox-open');
    document.body.classList.remove('modal-open', 'gallery-lightbox-open');
  };
  const close = () => {
  ...
  items.forEach((item, index) => item.addEventListener('click', () => { trigger = item; show(index); }));
  prevButton.addEventListener('click', (event) => { event.stopPropagation(); show(currentIndex - 1); });
  nextButton.addEventListener('click', (event) => { event.stopPropagation(); show(currentIndex + 1); });
  zoomOutButton.addEventListener('click', () => setZoom(zoom - .5));
  zoomInButton.addEventListener('click', () => setZoom(zoom + .5));
  ...
  lightbox.addEventListener('touchend', (event) => {
    ...
    if (zoom === 1 && Math.abs(distanceX) > 45 && Math.abs(distanceX) > Math.abs(distanceY)) show(currentIndex + (distanceX < 0 ? 1 : -1));
    touchStartX = null;
    touchStartY = null;
  }, { passive: true });
  window.galleryKeydownHandler = (event) => {
...
templates['proyecto-educativo'] = () => page(`<div class="education-hero"><div><p class="eyebrow">Escuela · Proyecto educativo</p><h1>Aprender para<br><span>tocar juntos.</span></h1><p>En nuestra escuela la música se aprende haciendo: escuchando, practicando y compartiendo. Cada alumno avanza a su ritmo, pero nadie aprende solo.</p></div><div class="education-hero-note"><span>Una escuela viva</span><strong>Escuchar.<br>Practicar.<br>Compartir.</strong><small>Desde la primera nota hasta el escenario.</small></div></div><div class="education-intro"><div><p class="eyebrow">Una manera de enseñar</p><h2>La primera nota importa.<br>El camino, mucho más.</h2></div><div><p>Nuestro proyecto educativo nace de una idea sencilla: la música tiene que ser una puerta abierta. Por eso acompañamos a niños, jóvenes y adultos con una formación cercana, progresiva y conectada con la vida de la banda.</p><p>No buscamos formar solo instrumentistas. Queremos que cada alumno aprenda a escuchar, a expresarse y a disfrutar de lo que construye con los demás.</p></div></div><div class="education-pillars"><div class="education-section-head"><div><p class="eyebrow">Lo que nos guía</p><h2>Aprender también es<br><span>encontrar tu lugar.</span></h2></div><p>Cuatro ideas que están presentes en las clases, los ensayos y cada concierto.</p></div><div class="education-pillar-grid">${EDUCATION_PILLARS.map((pillar) => `<article class="education-pillar"><span>${pillar.number}</span><h3>${pillar.title}</h3><p>${pillar.text}</p></article>`).join('')}</div></div><div class="education-route"><div class="education-section-head"><div><p class="eyebrow">Un recorrido progresivo</p><h2>De descubrir a<br><span>compartir.</span></h2></div><p>La formación se adapta a cada edad y nivel, pero siempre mantiene una dirección: hacer música con sentido y con otras personas.</p></div><div class="education-stage-list">${EDUCATION_STAGES.map((stage) => `<article class="education-stage"><div class="education-stage-number">${stage.number}</div><div><span>${stage.label}</span><h3>${stage.title}</h3><p>${stage.text}</p></div></article>`).join('')}</div></div><div class="education-community"><div><p class="eyebrow">Una escuela conectada</p><h2>La clase continúa<br><span>cuando salimos de ella.</span></h2><p>Los conciertos, los encuentros y la Banda Joven ayudan a que lo aprendido se convierta en experiencia. Las familias también forman parte: acompañan, animan y hacen posible que la música tenga un lugar en la vida del pueblo.</p></div><div class="education-actions"><a class="btn btn-primary" href="#matricula">Conocer la matrícula ↗</a><a class="btn btn-light" href="#banda-joven">Ver Banda Joven</a></div></div>`, 'page education-page');

function getAssociationYears(date = new Date()) {
  const foundationYear = ASSOCIATION_FOUNDATION_YEAR;
...
function setupDonationForm() {
  ...
  form.addEventListener('submit', event => {
    ...
    const subject = 'Solicitud de donación';
    const body = `Me gustaría realizar una donación de ${amount.toFixed(2).replace('.', ',')} €. Por favor, indicadme cómo puedo hacerla.`;
...
function removeButtonArrows(root = document) {
  root.querySelectorAll('a.btn, a.header-cta, button').forEach((button) => {
...
function syncLegalDetails() {
  ...
  legal.querySelector('.legal-note')?.remove();
  ...
  if (heading?.textContent.includes('Responsable')) {
    legal.querySelectorAll('.legal-external-services').forEach(node => node.remove());
    const cookiesHeading = [...legal.querySelectorAll('h2')].find(node => node.textContent.includes('Cookies y servicios externos'));
    const cookiesText = cookiesHeading?.nextElementSibling;
    if (cookiesText) {
      cookiesText.textContent = 'La matrícula se gestiona mediante un formulario incrustado de Google Forms. La web también carga tipografías de Google Fonts, imágenes de Unsplash y consulta puntualmente la agenda pública de Glissandoo. Estos servicios pueden tratar datos técnicos conforme a sus propias políticas.';
      cookiesText.insertAdjacentHTML('afterend', '<h2>Servicios externos</h2><p class="legal-external-services">Antes de enviar datos en un servicio externo, revisa su política de privacidad. Para consultas sobre el tratamiento realizado por la asociación puedes escribir a <a href="mailto:bandalamatanza@gmail.com">bandalamatanza@gmail.com</a>.</p>');
    }
  }
}
...
function render() {
  const { requestedKey, key } = resolveRoute();
  if (requestedKey !== key && location.hash !== `#${key}`) history.replaceState(null, '', `#${key}`);
  const html = templates[key]
    ? templates[key]()
    : page(title('Página no encontrada', 'Este camino no existe.', 'Vuelve al inicio o utiliza el menú para continuar.') + '<div class="not-found-actions"><a class="btn btn-primary" href="#inicio">Volver al inicio</a></div>');
  ...
  if (key === 'solicitud-matricula') document.querySelector('.enrollment-panel-head')?.remove();
  if (window.presidentsCarouselTimer) {
  ...
  if (window.announcementTimer) {
    ...
    window.announcementTimer = null;
  }
  if (key === 'inicio') {
    setupHomeAnnouncements();
    const homeContent = document.querySelector('.home-band')?.nextElementSibling;
    homeContent?.querySelector(':scope > .section-head')?.remove();
    homeContent?.querySelector(':scope > .cards')?.remove();
  }
  if (key === 'junta-directiva') {
    ...
    let cards = track ? [...track.children] : [];
    ...
    const presidentData = [];
    presidentData.splice(0, presidentData.length, ...VERIFIED_PRESIDENTS.map(([period, name, description]) => [period, name, description]));
    presidentImages.splice(0, presidentImages.length, ...VERIFIED_PRESIDENTS.map(([, , , image]) => image));
    if (track) {
      track.innerHTML = presidentData.map(() => '<article></article>').join('');
  ...
  if (associationYears && key === 'nuestra-historia') {
    const years = getAssociationYears();
    associationYears.textContent = years;
    const historyIntro = document.querySelector('.history-intro > div:first-child > p:not(.eyebrow)');
    if (historyIntro) historyIntro.textContent = `Lo que comenzó con la ilusión de acercar la música a los niños y jóvenes de La Matanza ha crecido hasta convertirse en un proyecto compartido por músicos, familias y vecinos. ${years} años de aprendizaje, conciertos y momentos que forman parte de la historia de nuestro pueblo.`;
  ...
  if (key === 'donaciones') {
    ...
    document.querySelector('.donation-intro p')?.remove();
    document.querySelector('.donation-intro .text-link')?.remove();
    document.querySelector('.donation-intro .donation-back')?.remove();
  }
